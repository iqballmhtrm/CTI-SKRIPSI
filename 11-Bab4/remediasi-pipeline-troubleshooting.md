...
| Hydra (T1110) | ambang `10/10s` terlalu ketat (Hydra ~17–20 percobaan/menit) | turunkan ke **`count 5/60s`** (rev:2) | eve.json + ES sid 1000020 (count=3) |
...
| Hydra (11–20) | T1110     | **1,6 dtk** (1–4) | **5,3 dtk** (2–9) | ya — Wazuh 5763 (sshd brute) → firewall-drop |
...
- Rule Nikto berbasis perilaku (laju permintaan) mendeteksi **aktivitas
  ...
  pemetaan T1595.002.
- Status `NO_MITIG` pada Nmap/Nikto adalah hasil sah: sistem memang hanya
  melakukan auto-respons (firewall-drop) terhadap brute force SSH.
