...
if test -f /var/log/suricata/eve.json; then
  grep -o '"signature_id":[0-9]*' /var/log/suricata/eve.json 2>/dev/null | grep -o '[0-9]\+' | sort -n | uniq > /tmp/observed_sids.txt || true
...
if test -f /etc/logstash/dictionaries/mitre-mapping.yml; then
  grep -oP '"\K[0-9]{4,7}(?="\s*:\s*" )' /etc/logstash/dictionaries/mitre-mapping.yml 2>/dev/null | sort -n > /tmp/mapped_sids.txt || true
  # Fallback if -P not supported
  if [ ! -s /tmp/mapped_sids.txt ]; then
    grep -o '"[0-9]\{4,7\}"\s*:' /etc/logstash/dictionaries/mitre-mapping.yml 2>/dev/null | grep -o '[0-9]\+' | sort -n > /tmp/mapped_sids.txt || true
  fi
